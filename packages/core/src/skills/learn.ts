 ...
 * feature used by both CLI TUI and VS Code: extension.
 *
 * The flow is:
 *
 *   1. Host runs `shouldOfferLearn(rounds, errors, user)` after each turn.
 *   2. If true, host calls a small reflection sub-call against the
 *      active provider with `buildReflectionMessages(...)`. The model is
 *      asked to either output a draft `LearnedSkillDraft` JSON or `null`.
 *   3. Host parses the response with `parseLearnedSkillDraft`.
 *   4. If a draft was produced, host shows a confirmation UI so the user
 *      can Save / Edit / Discard.
 *
 ...
 */
const contentToString = (content: string | readonly unknown[]): string =>
...
export const buildReflectionMessages = (
  ...
  const recent = history
    ...
    .flatMap<Message>((m) => {
      ...
      return [{ role: 'user', content: trim(contentToString(m.content)) }];
    });
